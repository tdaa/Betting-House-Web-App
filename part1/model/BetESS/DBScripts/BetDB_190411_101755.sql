...
alter table `user_group`   add index fk_user_group_user (`user_oid`), add constraint fk_user_group_user foreign key (`user_oid`) references `user` (`idutilizador`);
...
-- User_Aposta [rel2]
alter table `aposta`  add column  `user_idutilizador`  integer;
alter table `aposta`   add index fk_aposta_user (`user_idutilizador`), add constraint fk_aposta_user foreign key (`user_idutilizador`) references `user` (`idutilizador`);
